...
#+begin_src emacs-lisp :tangle yes
  (use-package fzf-native
    :vc (:url "https://github.com/jojojames/fzf-native" :rev :newest)
    :config
    (fzf-native-load-dyn))
#+end_src
...
| ~fzf-async-max-line-length~    | ~t~       | Per-line character limit; nil/t/+N/-N (see note below). |
| ~fzf-async-cache-size~         | 20        | LRU cache entries per session; increase for heavy backtracking. |
...
| ~t~    | Apply a built-in default of 512 characters (default). |
...
#+begin_src emacs-lisp :tangle yes
  (setq fzf-async-max-line-length t)     ; exclude lines > 512 chars
...
stays responsive with *millions* of candidates.  The goal is to match or
exceed the performance of ~fzf~ in the terminal from within Emacs.
