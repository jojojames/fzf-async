...
*Highlighting caveat:* match highlighting does not work perfectly *yet*
with ivy.  Ivy bypasses ~completion-styles~ entirely and does not call
the ~all-completions~ function where ~fzf-async~ applies face properties
to matched characters.
...
| ~fzf-async-input-debounce~    | 0.1       | Idle seconds to retry after input.                  |
| ~fzf-async-input-throttle~    | 0.2       | Min seconds between refreshes.                      |
...
~fzf-async-highlight~ controls whether matched characters are highlighted in
the completion list.  Highlighting requires a second pass over the candidate
list in Elisp (via ~completion-pcm--hilit-commonality~) after the C scoring
pass.  For very large candidate sets or slower machines this can be a
noticeable cost.  Set it to ~nil~ for plain output with no extra Elisp work:

#+begin_src emacs-lisp :tangle yes
  (setq fzf-async-highlight nil)
#+end_src
