...
Default value of ~fzf-async-extensions~ is ~(pass spotlight)~.  Soft dependencies
are not declared in ~Package-Requires~ — install them yourself if you use
the extension, they are loaded lazily on first call:
...
| ~fzf-async-extensions~         | ~(pass spotlight)~ | Extensions to load from ~extensions/~ during setup. |
